...
public class JLinkedInGetTest {
	...
	public void testGetCompanyUpdates() {
		JLinkedIn jli = new JLinkedIn().setAuthToken(JLinkedInTest.TEST_OAUTH_TOKEN_SPOON);
		List<LIUpdate> updates = jli.get().getCompanyUpdates(socialCutleryId, null, null, null);
	...
	public void  testGetCompanyUpdateComments() {
		JLinkedIn jli = new JLinkedIn().setAuthToken(JLinkedInTest.TEST_OAUTH_TOKEN_SPOON);
		List<LIUpdate> comments = jli.get().getCompanyUpdateComments(socialCutleryId, forkDayUpdateKey);
		boolean gotComments = false;
		if(comments != null) {
			for(LIUpdate comment: comments) {
